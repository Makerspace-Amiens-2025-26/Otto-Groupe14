---
layout: default
title: Etape 3 - Programmation
parent: Fabriquer Notre Robot !
nav_order: 3
---

# Étape 4 - Programmation (Le Cerveau du Robot)

Maintenant que le robot Otto-MKS est assemblé et optimisé mécaniquement, il est temps de lui donner vie. Dans cette étape, nous allons injecter le programme (en langage C++) dans la carte ESP32 à l'aide de l'IDE Arduino.

Notre code se démarque par l'abandon des mouvements saccadés au profit d'une marche fluide, et par l'utilisation de l'application **RemoteXY** pour un pilotage Bluetooth.

---

## 1. Comprendre les points clés du programme

Avant de copier le code complet, voici les trois concepts majeurs que nous avons implémentés pour rendre notre robot "TigrOtto" performant :

### A. La connexion Bluetooth via RemoteXY
RemoteXY génère automatiquement l'interface de notre télécommande. Dans le code, vous trouverez le nom de notre robot et son mot de passe de connexion :
```cpp
#define REMOTEXY_BLUETOOTH_NAME "TigrOtto"
#define REMOTEXY_ACCESS_PASSWORD "tigre"
```
Plus bas, une structure (`struct`) récupère en temps réel les actions de l'utilisateur sur son téléphone (le joystick, le bouton de l'arme et l'interrupteur auto/manuel).

### B. Le mouvement
Pour aller vite sans faire glisser les pieds du robot, nous avons banni la fonction `delay()` qui endort la carte. À la place, nous utilisons un chronomètre (`millis()`) couplé à des ondes en **Sinus** et **Cosinus**. 
Cela permet de basculer le poids (`ondePied`) et d'avancer les jambes (`ondeJambe`) de manière continue. Nous y avons aussi intégré notre fameuse variable `drift` pour corriger la trajectoire :
```cpp
  // Les deux jambes poussent vers l'arrière pour avancer
...
```cpp
    ...
    int seuil = 50;
```
Le robot ignorera tout mouvement inférieur à 50, évitant ainsi de lancer un virage accidentel.

---

## 2. Création du fichier "notes.h" (Pour la musique)

Pour que notre robot joue sa musique de démarrage (`startupMusic`), le buzzer a besoin de connaître la fréquence mathématique des notes. Plutôt que d'encombrer notre programme principal, ces fréquences sont rangées dans un fichier annexe appelé `notes.h`.

**Instructions pour créer ce fichier :**
1. Dans l'IDE Arduino, cliquez sur les trois petits points (ou la petite flèche vers le bas) situés complètement en haut à droite de l'écran, juste au-dessus de l'éditeur de texte.
2. Cliquez sur **Nouvel Onglet** (ou *New Tab*).
3. Nommez ce fichier exactement : `notes.h` et appuyez sur Entrée.
4. Un nouvel onglet vierge apparaît à côté de votre code principal. Copiez-collez ce petit bout de code dedans :

```cpp
// Fichier notes.h - Fréquences pour le buzzer
#ifndef NOTES4_H
#define NOTES4_H
...
## 3. Téléversement du Code Complet

Voici le code intégral de notre robot. 

**Instructions :**
1. Ouvrez l'IDE Arduino.
2. Assurez-vous d'avoir installé les bibliothèques `ESP32Servo` et `RemoteXY`.
3. Branchez votre robot en USB, sélectionnez le port correspondant à l'ESP32.
4. Copiez-collez le code ci-dessous et cliquez sur **Téléverser**.
...
> 💡 **Une fois le téléversement terminé :** Ne débranchez pas tout de suite ! Passez à la page **Contrôle & Test** pour effectuer les derniers tests de diagnostic et configurer l'application sur votre téléphone.